...
**Versione prompt attuale:** v1-final + patch v2/v2.1/v2.2 (in `prompts/system_prompt.md`; spec: **v4**)
...
| 2026-05-29 | Allineamento codice ↔ spec + SPEC_ERRATA | Creato `SPEC_ERRATA.md` (evoluzione v1→v3 + 9 voci errata). Test di verifica: 4/4 unit test privacy pass, redazione PII reale zero-leak (ERR-01 confermato), app FastAPI carica. Prompt text **invariato**. Scoperte 2 divergenze runtime non legate al prompt: Claude via CLI non SDK (ERR-08), gate lingua `langdetect` non implementato/post-LLM (ERR-09). +INC-005, +INC-006, corretta nota INC-004. |
| 2026-05-29 | Form consegna + OPF investigation + pivot Hybrid + code review | Compilato form consegna corso (domande 2-8). Investigato `openai/privacy-filter`: verifica repo (solo 3 commit, sperimentale), 3 query Perplexity (P1: OPF ~95-97% F1 su italiano vince su Presidio; P2: gira CPU-only ma ~2-3 samples/sec; P3: redazione PII pre-LLM è **requisito GDPR Art. 5/25**, DPA+7gg insufficienti da soli). Smoke test OPF su 2 PDF reali (NDA template: 83.5s/0 span; Locazione INPS 3K: 64.7s/6 span, CF+indirizzi rilevati ✅). Path ONNX abbandonato (architettura custom MoE+Triton, no onnxruntime Python nativo). **PIVOT deciso: OPF→Hybrid (regex IT + spaCy `it_core_news_sm`) per latency <1s.** Privacy Filter Integration.md aggiornato + linkato Progettistica AI MOC. Sessione successiva: scoperta cartella `prog1/27-05/specter-ai/` con MVP completo (18 round building 2026-05-20) + `CODE_REVIEW_SPECTERAI_20260521.md` con 5 bug critici → INC-007→011 registrati. **Rettifica:** verifica sul codice canonico (`prog1/specterai/specter-ai/`) → i 5 bug risultano **già risolti** nella sessione fix del 21/05 (commit `3b81fd1`…`999071f`); i numeri di riga citati (162/163/231/302) non esistono nei file attuali (118/140/87 righe). INC-007→011 riallineati a ✅ Risolto. Cartella `27-05/` duplicata eliminata (backup in `27-05.zip`). |
| 2026-05-29 | Config backend LLM (cli/sdk) — chiude ERR-08 | Aggiunti `config.py` (carica `.env`, switch `LLM_BACKEND`), `setup.py` (setup guidato: scelta backend → CLI install/login o API key → check/download spaCy `it_core_news_sm`), `.env.example`; `llm_client.py` ora dispatcha `_call_cli`/`_call_sdk` (SDK con `temperature=0`/`max_tokens=2048`, recuperati da ERR-08); `main.py` chiama `config.validate()`. Test: import chain OK, 4/4 unit test verdi. ERR-09 deciso (opzione B, spec→codice). INC-005 chiuso. |
| 2026-05-29 | **Spec v4** | ERR-01 (redazione PII) ed ERR-08 (backend cli/sdk) riconosciute come **cambi architetturali** → creata `Specifica Tecnica v4 - SpecterAI.md` (self-contained): +§3.bis privacy-first GDPR Art.5/25, +backend configurabile, gate lingua post-LLM, schema multi-span, T13/T14/T15. Prompt text invariato. v3.1 congelata e preservata; SPEC_ERRATA = driver documentato della v4. |
