<?php
/**
 * WooCommerce single-file router — design Bibliò per tutte le pagine WC.
 *
 * WordPress carica questo file invece dei template WC standard quando esiste
 * nel root del tema. Gestiamo qui: shop archive, product category, single product.
 * Cart / Checkout / My Account restano gestiti da WC (mostrati via [shortcode] sulle pagine).
 */
...
if (is_singular('product')):
    /* =========================================================
       SINGLE PRODUCT — design come single-book.php
       ========================================================= */
      ...
      <div>
        ...
        <div class="buyrent-price buy">
          ...
          if ($sale && $regular && $sale < $regular) {
              echo '<span style="text-decoration:line-through;color:var(--fg-muted);font-weight:400;font-size:20px;margin-right:10px;">' . biblio_price($regular) . '</span>';
              echo biblio_price($sale);
          } else {
              echo biblio_price($product->get_price());
          }
          ?>
        </div>
...
elseif (is_shop() || is_product_taxonomy()):
    /* =========================================================
       SHOP / CATEGORY ARCHIVE — design come archive-book.php
       ========================================================= */
...
<?php
    ...
    /* Fallback per altre pagine WC (cart/checkout/account) — usa template WC standard */
