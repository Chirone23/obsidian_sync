...
<section class="hero">
  <div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;">
      ...
      <a class="btn btn-secondary btn-lg" href="<?php echo esc_url(home_url('/mybiblio/')); ?>">✨ Chiedi a MyBibliò</a>
    </div>
  </div>
...
<section class="section">
  ...
  <div class="grid-6">
    <?php
    $q = $use_wc ? biblio_products_query(['posts_per_page' => 6]) : biblio_books_query(['posts_per_page' => 6]);
    if ($q->have_posts()) while ($q->have_posts()) { $q->the_post(); $render_card(get_the_ID(), true); }
    wp_reset_postdata();
    ?>
  </div>
</section>
