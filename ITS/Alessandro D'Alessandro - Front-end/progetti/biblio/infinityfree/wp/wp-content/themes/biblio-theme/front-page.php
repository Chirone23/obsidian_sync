...
<section class="hero">
  ...
  <div class="hero-stack">
    ...
    if ($featured->have_posts()) {
        while ($featured->have_posts()) { $featured->the_post(); $covers[] = get_the_ID(); }
        wp_reset_postdata();
    }
    if (count($covers) >= 3) :
    ?>
      <div class="cover cover-1"><?php biblio_product_cover($covers[1]); ?></div>
      <div class="cover cover-2"><?php biblio_product_cover($covers[0]); ?></div>
      <div class="cover cover-3"><?php biblio_product_cover($covers[2]); ?></div>
    <?php endif; ?>
  </div>
</section>
...
<section class="section">
  <div class="section-head">
    ...
    <a class="btn btn-ghost" href="<?php echo esc_url($shop_url); ?>">Vedi tutti &rarr;</a>
  </div>
  <div class="grid-6">
    <?php foreach ($covers as $fid) { biblio_product_card($fid, true); } ?>
  </div>
</section>
...
<section class="plus-banner">
  ...
  <a class="btn btn-lg btn-gold" href="<?php echo esc_url(home_url('/plus/')); ?>">Attiva Plus &rarr;</a>
</section>
...
<section class="section">
  <div class="section-head">
    ...
    <a class="btn btn-ghost" href="<?php echo esc_url($shop_url); ?>">Tutte le novità &rarr;</a>
  </div>
  <div class="grid-6">
    <?php
    if ($news_q->have_posts()) {
        while ($news_q->have_posts()) { $news_q->the_post(); biblio_product_card(get_the_ID(), true); }
        wp_reset_postdata();
    }
    ?>
  </div>
